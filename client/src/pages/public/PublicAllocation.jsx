...
function DateCard({ label, value }) {
  return (
    <div className="text-center">
      <p className="text-xs uppercase tracking-wide mb-1" style={{ color: '#A8896A' }}>{label}</p>
      <p className="text-sm" style={{ color: '#221508', fontWeight: 500 }}>{value}</p>
    </div>
  );
}

export default function PublicAllocation() {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetch(`/api/public/allocations/${id}`)
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
      .then(d => setData(d))
      .catch(() => setError('This allocation could not be found.'))
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#FDFAF6', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#A8896A', fontSize: 14 }}>{error || 'Allocation not found.'}</p>
        </div>
      </div>
    );
  }
        ...
        {journal.length === 0 && !label && (
          ...
          >
            <p style={{ color: '#A8896A', fontSize: 14, margin: 0 }}>
              Documentation for this allocation is being prepared.
            </p>
          </div>
        )}
