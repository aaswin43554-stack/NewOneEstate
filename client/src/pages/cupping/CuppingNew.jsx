...
function ScoreSummary({ scores }) {
  ...
  return (
    <div className="text-center">
      <p className="text-coffee-900" style={{ fontSize: 56, fontWeight: 500, lineHeight: 1 }}>{total}</p>
...
function useVoiceInput(onResult) {
  ...
  function startListening(attrKey) {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    ...
    rec.onstart  = () => setListening(attrKey);
    rec.onend    = () => setListening(false);
    rec.onerror  = () => setListening(false);
    rec.onresult = (e) => {
      const transcript = e.results[0][0].transcript;
      onResult(attrKey, transcript);
    };
...
export default function CuppingNew() {
  ...
  async function handleAiStructure() {
    const hasAny = ATTRS.some(a => obs[a.key]?.trim());
    if (!hasAny) { setAiError('Enter at least one observation note before structuring.'); return; }
    setAiLoading(true); setAiError('');
    try {
      const res = await api.post('/ai/cupping-structure', { obs });
      ...
      <div className="max-w-2xl mx-auto px-6 py-6">
          ...
          {steps.map((label, i) => {
            ...
            return (
              <div key={label} className={`flex items-center ${i < steps.length - 1 ? 'flex-1' : ''}`}>
                ...
                {i < steps.length - 1 && (
                  <div className="flex-1 mx-3 mb-5" style={{ height: 1, background: step > s ? '#533A24' : '#E0D0BC' }} />
                )}
              </div>
            );
          })}
        ...
        {step === 1 && (
          <div className="space-y-5">
            <h2 className="text-base text-coffee-900" style={{ fontWeight: 500 }}>Select Roast Session</h2>
            ...
            <Button variant="primary" onClick={() => setStep(2)} disabled={!roastSessionId || !roastSession?.ended_at} className="w-full justify-center">
              Continue
            </Button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            <h2 className="text-base text-coffee-900" style={{ fontWeight: 500 }}>Session Details</h2>
            ...
            <FormInput label="Cupping Date" type="date" value={cupDate} onChange={e => setCupDate(e.target.value)} />
            ...
            <div className="flex gap-3">
              ...
              <Button variant="primary" onClick={() => setStep(3)} disabled={!purpose} className="flex-1 justify-center">
                Continue
              </Button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-base text-coffee-900" style={{ fontWeight: 500 }}>Score Sample</h2>
              <Button
                variant="secondary"
                size="sm"
                onClick={handleAiStructure}
                disabled={aiLoading}
                title="Use AI to structure your tasting notes"
              >
            ...
            {aiError && (
              <p className="text-xs" style={{ color: '#A32D2D' }}>{aiError}</p>
            )}
                ...
                return (
                  <div key={key} className="bg-white border border-coffee-200 rounded-xl px-4 py-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="font-mono text-coffee-800" style={{ fontSize: 20, fontWeight: 500, minWidth: 24, textAlign: 'right' }}>
                        {scores[key]}
                      </span>
                    </div>
                    <input
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-2 py-1.5 text-sm border border-coffee-100 rounded-lg focus:border-coffee-300 focus:ring-1 focus:ring-coffee-100 text-coffee-700"
                      />
                      {voiceSupported && (
                        <button
                          type="button"
                          ...
                          style={{
                            background: isActive ? '#FCEBEB' : '#F2EAE0',
                            color:      isActive ? '#A32D2D' : '#8B6A47',
                          }}
                        >
                    ...
                    {isActive && (
                      <p className="text-xs mt-1" style={{ color: '#A32D2D' }}>Listening…</p>
                    )}
                  </div>
                );
              })}
            ...
            {/* Decision */}
            <div>
              <p className="text-sm text-coffee-600 mb-2" style={{ fontWeight: 500 }}>Final Decision</p>
              <div className="grid grid-cols-3 gap-2">
                {[
            ...
            <div className="flex gap-3">
              ...
              <Button variant="primary" onClick={handleSubmit} disabled={saving} className="flex-1 justify-center">
